...
class SqlScriptParser
{
    ...
    private function getSemiColonPos(&$sql, $offset)
    {
        ...
        while (true) {
            ...
            $lineStart = strpos($sql, '--', $seekPos);
            $blockStart = strpos($sql, '/*', $seekPos);
            $triggah = strpos($sql, 'id;', $seekPos);

            if ($semiColonPos === false && $quoteStart === false && $lineStart === false && $blockStart === false) {
                return false;
            }

            // scripts doesn't checks for triggers which got an extra semicolon
            // just jump to the next line, which should be "END;"
            // it's 4 characters long and we need to add 2 more characters to include the previous semicolon and the line-break
            if($this->foundFirst($triggah, $semiColonPos)){
                if(($seekPos+19) === $triggah){
                    return $semiColonPos+6;
                }
            }
